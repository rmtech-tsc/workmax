 ...
 */
 ...
 export const updateEmployeePersonalEmailInDB = async (
   employeeId,
   personalEmail
 ) => {
   if (!supabase) throw new Error('Supabase not configured');
 ...
 export const updateEmployeeFaceInDB = async (
   employeeId,
   faceImage,
   faceDescriptor
 ) => {
 ...
 export const updateLeaveInDB = async (
   leaveId,
   status,
   adminComment,
   reviewedBy
 ...
 /** Save an attendance record. Silent fail so check-in still works offline. */
 export const createAttendanceInDB = async (record, companyId) => {
   ...
   try {
     await supabase.from('attendance').insert({
       ...
       location_verified: record.locationVerified || false,
     });
   } catch {
     /* silent */
   }
